...
export function convertToHandle(str) {
 return str;
  /*
  return str
    .toLowerCase()
    .trim()
    .replace(/[^a-z0-9]+/g, '-') // replace any sequence of disallowed chars with a single dash
...
export function productType() {
  const productType = process.env.PUBLIC_PRODUCT_TYPE; // Fetching MY_VARIABLE from .env file or environment
  return { productType }; // Returning it as a property of an object
}

export function formatString(string) {
  return string.split(/[\s-:_]+/).map((s) => s[0].toUpperCase() + s.slice(1)).join(' ')
}
